 ...
 * @file projets.js
 * * Correctifs SPA appliqués :
 * - IIFE englobante
 * - Protection des event listeners window
 */
...
(() => {
  ...
  function initModals() {
    ...
    if (modalEdit && !modalEdit.dataset.eventsBound) {
      ...
      modalEdit.dataset.eventsBound = '1';
    }

    const modalDelete = document.getElementById('modalDeleteProject');
    if (modalDelete && !modalDelete.dataset.eventsBound) {
      ...
      modalDelete.dataset.eventsBound = '1';
    }
  }
  ...
  function initSearch() {
    ...
    // Nettoyage de l'ancien listener en clonant l'input
    ...
    function buildSuggestions(data) {
      ...
      const container = freshInput.closest('search') ?? freshInput.parentElement;
    ...
    freshInput.addEventListener('blur',    () => setTimeout(removeSuggestions, 150));
    freshInput.addEventListener('keydown', e => { if (e.key === 'Escape') { removeSuggestions(); freshInput.blur(); } });
  }

  /* ════════════════════════════════════════════════════
   * INIT & SECURITE SPA
   * ════════════════════════════════════════════════════ */
  window.YesPageInit = function projetsInit() {
    initModals();
    drawProjetGantt();
    initSearch();
  };
