 ...
 *  - Planning global : Gantt + Calendrier
 */
...
(function () {
  ...
  /* ── Attacher un listener une seule fois via data-flag ── */
  function attachOnce(el, event, handler) {
    if (!el || el.dataset['_' + event + '_bound']) return;
    el.dataset['_' + event + '_bound'] = '1';
  ...
  function _pageInit() {
    ...
    /* FIX pagination : retirer tout display inline pour laisser le JS contrôler */
    if (paginationNav) {
    ...
    function applyFilters() {
      ...
      // Réinsérer les éléments dans le DOM dans le bon ordre
    ...
    document.querySelectorAll('[data-todo-filter]').forEach(btn => {
      /* Retirer ancien listener via cloneNode propre */
    ...
    if (sortEl) {
      ...
      /* Forcer la valeur à 'default' sur le nouveau nœud */
  ...
  window.openPgEdit = function (data) {
    ...
    s('pge-id', data.id); s('pge-titre', data.titre); s('pge-statut', data.statut||'wip');
  ...
  function drawPgGantt() {
    ...
    tasks.forEach((t,i)=>{const y=24+i*RH,c=t.couleur||'#0d6efd';ctx.fillStyle=isDark?(i%2===0?'#1e1e2e':'#252535'):(i%2===0?'#fff':'#f8f9fa');ctx.fillRect(0,y,W,RH);ctx.fillStyle=isDark?'#e9ecef':'#212529';ctx.font='12px system-ui,sans-serif';ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillText(_tr(ctx,t.titre||'—',LW-PAD*2),PAD,y+RH/2);const db=new Date(t.date_debut).getTime(),fn=t.date_fin?new Date(t.date_fin).getTime():db+86400000,bx=LW+PAD+((db-minTs)/totalMs)*CW,bw=Math.max(4,((fn-db)/totalMs)*CW);ctx.shadowColor='rgba(0,0,0,.12)';ctx.shadowBlur=4;ctx.shadowOffsetY=2;_rr(ctx,bx,y+(RH-BH)/2,bw,BH,5,c);ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.shadowOffsetY=0;if(bw>40){ctx.fillStyle='#fff';ctx.font='bold 9px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(PG_LABELS[t.statut]||t.statut,bx+bw/2,y+RH/2);}});
  ...
  function renderPgCalendar(){
    ...
    for(let day=1;day<=dim;day++){
      ...
      dt.slice(0,2).forEach(t=>{const bg=t.couleur||PG_COLORS[t.statut]||'#0d6efd';const p=document.createElement('div');p.style.cssText=`background:${bg}33;border-left:3px solid ${bg};border-radius:3px;padding:1px 4px;margin-bottom:2px;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:${isDark?'#e9ecef':'#212529'};font-weight:500;`;p.textContent=t.titre||'—';cell.appendChild(p);});
  ...
  function showPgDay(day,ds,tasks){
    ...
    list.innerHTML='';
    tasks.forEach(t=>{const c=t.couleur||PG_COLORS[t.statut]||'#0d6efd',l=PG_LABELS[t.statut]||t.statut;const li=document.createElement('li');li.className='list-group-item d-flex justify-content-between align-items-center py-2';li.innerHTML=`<div><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${c};margin-right:8px;"></span><strong>${_esc(t.titre||'—')}</strong>${t.description?`<br><small class="text-body-secondary ms-4">${_esc(t.description)}</small>`:''}</div><span class="badge rounded-pill" style="background:${c};">${_esc(l)}</span>`;list.appendChild(li);});
