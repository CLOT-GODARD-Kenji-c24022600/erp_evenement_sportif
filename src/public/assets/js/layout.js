...
(() => {
  ...
  if (window._yesLayoutInitDone) return;
  window._yesLayoutInitDone = true;

  function initLayout() {
    // ── Fix hauteur viewport ─────
    ...
    if (isMobile()) document.body.classList.remove('collapsed');
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && document.body.classList.contains('sidebar-open')) closeMobileSidebar();
    });
    ...
    window.addEventListener('resize', () => {
      if (!isMobile() && document.body.classList.contains('sidebar-open')) closeMobileSidebar();
    }, { passive: true });
    ...
    if (sidebar) {
      ...
      sidebar.addEventListener('touchend', (e) => {
        ...
        if (dx < -60 && Math.abs(dx) > Math.abs(dy)) closeMobileSidebar();
      }, { passive: true });
    }
    ...
    document.addEventListener('touchend', (e) => {
      ...
      if (dx > 60 && !document.body.classList.contains('sidebar-open')) openMobileSidebar();
      touchEdgeStart = null;
    }, { passive: true });
